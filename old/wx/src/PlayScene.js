...
var PlayLayer = cc.LayerColor.extend({
	...
	ctor:function () {
		//f8c34d
		this._super(cc.color(248,99,77,255),cc.winSize.width,cc.winSize.height);
		var size = cc.winSize;
		this.SushiSprites =[];
		this.BoomSprites=[];
		...
		////ffb1b1 标题
		var titleLayer = new cc.LayerColor(cc.color(161,63,50,255),size.width,size.height/8); 
		...
		logoSprite.setScale(size.width/logoSprite.getContentSize().width*0.7);
		...
		//ffb1b1 底部时间
		var underLayer = new cc.LayerColor(cc.color(161,63,50,255),size.width,size.height/8); 
	...
	addGameOverLayer:function(flag){
		...
		cc.log("game over");
		//ffc5c3
		var gameOverLayer = new cc.Layer();//new cc.LayerColor(cc.color(255,197,196,255));
		var size = cc.winSize;
		var size = cc. winSize;
		//add bg
		this.bgSprite = new cc.Sprite(res.Scorebg_jpg);
		this.bgSprite.attr({
			anchorX:0.5,
			anchorY:1,
			x:size.width/2,
		...
		gameOverLayer.addChild(this.bgSprite,8);
		
		
		var gameoversp = new cc.Sprite(res.Card_png);
		//gameoversp.setScale(0.5,0.5);
		gameoversp.setPosition(size.width/2, size.height/2);
		gameOverLayer.addChild(gameoversp,17);
		// 根据分数输出不同的语句
		var gameoverspSize = gameoversp.getContentSize();
		//抢到面膜
		var label1 = new cc.LabelTTF("抢到面膜",res.font,40)//, 
				//cc.size(gameoverspSize.width,gameoverspSize.height/3), cc.TEXT_ALIGNMENT_LEFT);
		label1.attr({
			anchorX:0.5,
			anchorY:1,
			x:label1.getContentSize().width*1.3,//gameoverspSize.width/2,
			y:gameoverspSize.height-label1.getContentSize().height
		});
		label1.	fillStyle=cc.color(255,255,255,255);
		gameoversp.addChild(label1,20);
		//分数
		...
		label2.attr({
			x:gameoverspSize.width/1.7,
			y:gameoverspSize.height/1.6
		});
		label2.	fillStyle=cc.color(247,67,50,255);
		gameoversp.addChild(label2,20);
		//个
		var label3 = new cc.LabelTTF("个",res.font,40)//, 
		//cc.size(gameoverspSize.width,gameoverspSize.height/3), cc.TEXT_ALIGNMENT_LEFT);
		label3.attr({
			anchorX:0.5,
			anchorY:1,
			x:gameoverspSize.width-4*label3.getContentSize().width,//gameoverspSize.width/2,
			y:gameoverspSize.height-label1.getContentSize().height
		});
		label3.	fillStyle=cc.color(255,255,255,255);
		gameoversp.addChild(label3,20);
		//有机会获得面膜
		var str1="";
		if(flag==3){
			str1="有机会获得免费面膜,恭喜你!";
		}else{
			str1="很遗憾不能领取免费面膜,再玩一次吧!";
		}
		...
		label4.attr({
			anchorX:0.5,
			anchorY:1,
			x:gameoverspSize.width/2,//gameoverspSize.width/2,
			y:label4.getContentSize().height+10
		});
		label4.fillStyle=cc.color(255,255,255,255);
		...
		if(flag==3){
			...
			var rewardY =underLayer.getContentSize().height;
			reward.attr({
				x: size.width/2,
				y: rewardY,
				anchorX: 0.5,
			...
			this.getParent().addChild(gameOverLayer,9);			
			...
			var tryY = underLayer.getContentSize().height;
			tryAgain.attr({
				x: size.width/2,
				y: tryY,
				anchorX: 0.5,
		...
		gameOverLayer.addChild(underLayer,9);
		cc.log("menu");
	}
